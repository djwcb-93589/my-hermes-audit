...
## P5 Background Review 本地事实回放

P5 Trace 使用 `p5` version，并只回放已持久化的 trial-local Review 事实：Review run、foreground evidence、Subject prepared evidence、Review tool observation、before/after snapshot、observed change、lifecycle attempt 及 Background Review evaluator。mapper 不重新创建 claim、不重新运行 ReviewAgentLoop、模型或工具，也不读取 MyHermes live state。

P5 一律使用 content-free projection：Review ID、evidence kind、hash、长度、顺序、来源关联、status、action、target type、revision hash、attempt、duration、gate 和安全 metadata。即使没有 `--langfuse-no-content`，也不上传 Review evidence、foreground message、Assistant decision/report、Tool 正文、Memory/Skill 正文、Review Prompt、隐藏推理、claim token 或凭据。`sensitive` 保持同一强制边界。

P5 six-dimension Metric 只作为 evaluator Observation；`review_gate_passed` 是 task gate 输入，不增加 `review_score`、`background_review_score` 或任何第四个一级 Score。详见 [P5 文档](p5-background-review.md)。
...
一个 Suite 对应一个 Dataset。非 P4 Case 内容版本仍对应一个 Dataset Item，Item ID 保持 `dataset_name + suite_id + case_id + case_sha256` 的稳定 UUID5；P4 在其中加入显式 `variant_id`，使每个 Case/Variant 独占 Item。P5 Dataset projection 只增加 Review Plan/expectation 的 kind、lifecycle、turn、次数、目标 ID 和安全布尔边界，并固定记录 evidence/Skill/Memory 正文未上传。公开 `create_dataset_item(..., id=...)` 提供同 ID 写入语义；Case hash 与发布投影 hash 都不变时不写，投影变化时更新同一版本，Case 内容变化时创建新的稳定版本。历史 Item 和历史 Experiment 保留，没有 prune 或删除命令。
...
`TrialResult` 映射为 Runner task Trace 的子树：Trial span、scripted turn、公共 Model/Tool Observation、Validator evaluator，以及实际完成的 Judge generation。P3 Memory Case 还增加 seed、query、before/after snapshot 和 retrieval evaluator Observation；P5 Case 增加内容省略的 Review run/evidence/tool/snapshot/change/lifecycle/evaluator Observation。它们只是本地事实回放，不执行第二次查询或评测。未执行的 Judge 不伪造 generation。真实 runtime duration 和可用时间只进入 metadata；发布发生时创建的 span 不伪装成历史瀑布图。
...
只发布 `task_success`、`tool_correctness`、`answer_quality`。P3 required evidence、Memory state、Recall@K、MRR 与 P5 Review 的六个维度只作为 Observation/本地 Metric，不增加 Score。Score ID 由 `trace_id + score_name + evaluator_version + trial_id` 的 SHA-256 派生；同一身份固定使用首次持久化的 timestamp 和 `value_hash`。Langfuse 官方 [Score 数据模型](https://langfuse.com/docs/evaluation/scores/data-model) 明确允许自定义 Score ID 作为幂等键更新同一个 Score。
