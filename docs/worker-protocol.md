...
## P5 Worker and P6.1 Worker v10

## P6.1 Worker v10

`myhermes-audit-worker-v10` adds strict `scenarios` plans to the request and
...
v10 also carries explicit Process cursor references, Process timing availability,
...
v10 additionally carries content-free Process event-alignment diagnostics and
...
Tool boundaries. The Worker uses the public PRE/POST Tool hooks for aggregate
monotonic boundaries; unavailable hook facts remain explicit. The effective
Worker case watchdog is carried as
`hard_timeout_source=worker_case_watchdog` and is independent of the
observation span. WAIT remaining-budget facts are either Worker-monotonic or
explicitly unavailable; an unavailable value is never marked as an exact
match. Parents strictly reject v9 envelopes instead of silently defaulting
these fields. Alignment records contain only safe indexes, public labels,
hashed identities, status, reason, and Step IDs. Cleanup timing remains a
Worker lifecycle fact and is not folded into the foreground Scenario timeout.
Only relative event offsets and monotonic duration cross the protocol; host
specific absolute monotonic nanoseconds do not.
Missing timestamps make the observation span unavailable rather than
successful.

Current default: `myhermes-audit-worker-v10`. P6.1 timing fields use the
explicit observation-span, hard-watchdog, and remaining-budget semantics above;
the former v9 wall-clock wording is historical and is not a current contract.

默认协议升级为 `myhermes-audit-worker-v10`。P5 request/result、P6.1 scenario Artifact 必须使用 v10，父子进程会严格拒绝不兼容版本或不完整的 Artifact 引用。P6.1 的 Process 结果显式区分事件对齐诊断、persistence observation span、公开 Hook monotonic offsets、Worker watchdog、fixture file-read 证据与 Agent close/Worker cleanup；缺少可靠边界不会被解释为精确成功。当前 Worker 仍可执行没有 P5/P6.1 Plan 的 P0–P5 Suite；它们不初始化 Review 或 Process 场景运行时，也不会携带对应结果。遗留 v9 envelope 不会被静默降级。
