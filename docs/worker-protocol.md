...
默认协议升级为 `myhermes-audit-worker-v11`。P5 request/result、P6.1 scenario Artifact 必须使用 v11，父子进程会严格拒绝 v10 或其他不兼容版本及不完整的 Artifact 引用。P6.1 的 Process 结果显式区分事件对齐诊断、persistence observation span、PRE/POST Hook 来源与相对偏移、Worker watchdog、fixture file-read 证据与 Agent close/Worker cleanup；缺少可靠边界不会被解释为精确成功。当前 Worker 仍可执行没有 P5/P6.1 Plan 的 P0–P5 Suite；它们不初始化 Review 或 Process 场景运行时，也不会携带对应结果。遗留 v10 envelope 不会被静默降级。
