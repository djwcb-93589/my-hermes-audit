...
## P5 Worker and P6.1 Worker v9

## P6.1 Worker v9

`myhermes-audit-worker-v9` adds strict `scenarios` plans to the request and
...
v9 also carries explicit Process cursor references, Process timing availability,
...
v9 additionally carries content-free Process event-alignment diagnostics and
scenario wall-clock timing facts: unexpected, missing, order-violation,
foreign-process, and unconsumed event lists; UTC start/end timestamps; the
wall-clock duration; and a separate Tool-duration sum. Parents strictly reject
v8 envelopes instead of silently defaulting these fields. Alignment records
contain only safe indexes, public labels, hashed identities, status, reason,
and Step IDs. Cleanup timing remains a Worker lifecycle fact and is not folded
into the foreground Scenario timeout. Missing timestamps make required
Scenario timing unavailable rather than successful.

默认协议升级为 `myhermes-audit-worker-v9`。P5 request/result、P6.1 scenario Artifact 必须使用 v9，父子进程会严格拒绝不兼容版本或不完整的 Artifact 引用。v9 的 Process 结果显式区分事件对齐诊断、可靠 wall-clock timing、fixture file-read 证据与 Agent close/Worker cleanup；缺少 timing 不会被解释为成功。当前 Worker 仍可执行没有 P5/P6.1 Plan 的 P0–P5 Suite；它们不初始化 Review 或 Process 场景运行时，也不会携带对应结果。遗留 v8 envelope 不会被静默降级。
