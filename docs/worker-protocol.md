...
## P5 Worker and P6.1 Worker v8

## P6.1 Worker v8

`myhermes-audit-worker-v8` adds strict `scenarios` plans to the request and
...
v8 also carries explicit Process cursor references, Process timing availability,
...
默认协议升级为 `myhermes-audit-worker-v8`。P5 request/result、P6.1 scenario Artifact 必须使用 v8，父子进程会严格拒绝不兼容版本或不完整的 Artifact 引用。v8 的 Process 结果显式区分可靠 timing、fixture file-read 证据与 Agent close/Worker cleanup；缺少 timing 不会被解释为成功。当前 Worker 仍可执行没有 P5/P6.1 Plan 的 P0–P5 Suite；它们不初始化 Review 或 Process 场景运行时，也不会携带对应结果。遗留 v7 envelope 不会被静默降级。
