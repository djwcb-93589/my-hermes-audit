...
支持的 Python SDK 范围是 `langfuse>=4.14.2,<5`，且只接受最终语义版本。项目下限要求同时具备当前适配器使用的 v4 Score 创建与精确查询公开表面；上界固定在下一主版本之前，避免在未知破坏性变更上继续写远端。
...
Dataset-backed Runner 在一个 run 内以 Dataset Item 作为 Experiment Item 身份。普通 Case 使用一个 Item；P4 Case 为每个显式 Variant 建立独立 Item。因此远端发布要求每个 Case/Variant 恰好一个 Trial，并在启动 Worker 前拒绝其他配置；无 `--langfuse` 的本地多 Trial 能力不受影响。
...
未安装、低于 4.14.2、达到 5.0.0、预发布版本或缺少必要公开能力时，显式 `--langfuse` 抛出 Audit 自有依赖/能力异常并返回非零。`doctor --check-langfuse` 报告安装版本、最低版本、兼容性、Experiment 策略、Score 幂等策略、各能力和缺失项；它只初始化并关闭 client，不做连接或资源创建。普通无 Langfuse 路径仍可运行。
